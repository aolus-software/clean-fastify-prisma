...
export class UserActivitiesRepository extends BaseRepository implements IUserActivitiesRepository {
	...
	async getActivitiesByUser(userId: string, limit = 50): Promise<IUserActivity[]> {
		return this.query<IUserActivity>(
			`SELECT * FROM user_activities
	...
	async insertActivity(activity: Omit<IUserActivity, "id" | "timestamp">): Promise<void> {
		await this.insert("user_activities", [activity]);
	}

	async insertBatch(activities: Omit<IUserActivity, "id" | "timestamp">[]): Promise<void> {
