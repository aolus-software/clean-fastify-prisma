...
export class UserActivitiesService {
	...
	async trackActivity(activity: Omit<IUserActivity, "id" | "timestamp">): Promise<void> {
		// Add business logic here (validation, enrichment, etc.)
		await this.repository.insertActivity(activity);
	}

	async trackActivitiesBatch(activities: Omit<IUserActivity, "id" | "timestamp">[]): Promise<void> {
		// Validate batch size
	...
	async getPopularActions(days = 7): Promise<{ action: string; count: number }[]> {
