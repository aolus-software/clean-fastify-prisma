...
export const translateValidationIssue = (
	...
	if (message.includes("expected string") && message.includes("less"))
		return { key: "validation.tooLong" };
	if (message.includes("expected number to be greater")) return { key: "validation.tooSmall" };
	if (message.includes("expected number to be less")) return { key: "validation.tooLarge" };
	if (message.includes("expected string")) return { key: "validation.expectedString" };
	if (message.includes("expected number")) return { key: "validation.expectedNumber" };
	if (message.includes("expected boolean")) return { key: "validation.expectedBoolean" };
	if (message.includes("expected array")) return { key: "validation.expectedArray" };
	if (message.includes("expected object")) return { key: "validation.expectedObject" };
