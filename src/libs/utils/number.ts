export class NumberToolkit {
	...
	static forHumans(number: number): string {
		return new Intl.NumberFormat(NumberToolkit.defaultLocale).format(number);
	}

	static format(
		number: number,
		decimals: number = 2,
		locale: string = NumberToolkit.defaultLocale,
	): string {
	...
	static spell(number: number): string {
		const spellOut = new Intl.NumberFormat(NumberToolkit.defaultLocale, {
			style: "decimal",
		});
	...
	static useLocale(locale: string): void {
		NumberToolkit.defaultLocale = locale;
	}
	...
	static useCurrency(currencySymbol: string): void {
		NumberToolkit.defaultCurrency = currencySymbol;
	}

	static withCurrency(
		amount: number,
		currencySymbol: string = NumberToolkit.defaultCurrency,
		decimals: number = 2,
	): string {
		return NumberToolkit.currency(amount, currencySymbol, decimals);
	}
}
